...
std::vector<Square *> King::getAvailableSquares(bool considerCheck) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (squareShouldExist) {
      ...
      bool squareHasFriendlyPiece = square->getChessPiece() && square->getChessPiece()->getColor()==color;
    ...
    for (int i = 0; i < 2; i++) {
      ...
      for (int j = 1; j <= 4; j++) {
        ...
        if (neighbourXLocation==0 || neighbourXLocation==7) {
          if ((neighbour->getChessPiece()) && (neighbour->getChessPiece()->getType()==ROOK)
              && (neighbour->getChessPiece()->getAmountOfSteps()==0)) {
      ...
      if (castleAllowed && castlingRook) {
        Square *kingTargetSquare =
            board->getSquare(location->getCoordinates().x + (2*directions[i]), location->getCoordinates().y);
        Square *rookTargetSquare =
            board->getSquare(location->getCoordinates().x + (1*directions[i]), location->getCoordinates().y);
        moves.push_back(new Move(board,
                                 location,
                                 kingTargetSquare,
                                 false,
                                 castlingSides[i],
